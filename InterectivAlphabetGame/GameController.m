...
- (void)shakeLettersRandom{
    ...
    [self.task.words removeObjectAtIndex:randomIndex];
    ...
    for (int i = 0 ; i < lenght1 ; i ++) {
        ...
        if (![letter isEqualToString:@""]) {
            OCAIALetterView *letterView = [[OCAIALetterView alloc]initWithLetter:letter letterSize:letterSize];
    ...
    for (int i = 0 ; i < lenght1 ; i++) {
        ...
        if (![letter isEqualToString:@""]) {
            OCAIATargetLetterView *targetView = [[OCAIATargetLetterView alloc]initWithLetter:letter lenght:letterSize imageName:randomString1];
            ...
            for (OCAIALetterView *let in self.letters) {
                if ([let.letter isEqualToString:letterView.letter]) {
                    [UIView animateWithDuration:1 delay:0 options:UIViewAnimationOptionCurveEaseOut animations:^{
                        ...
                        letterView.center = CGPointMake(oldPoint, (kScreenHeight / 4 * 3));
                    } completion:nil
                     ];
                }
            }
